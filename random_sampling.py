# Execute in Terminal to render: python -m manim -pql manim/random_sampling.py Random_Sampling
import numpy as np
from manim import *
import numpy as np
...
class Random_Sampling(MovingCameraScene):
    def construct(self):
        ...
        rnd_ind = np.hstack((np.zeros(int(N/2)), np.ones(int(N/2))))

        intercept = 1

        y = intercept + beta * X + delta * rnd_ind + e
        ...
        self.play(data[0:(int(N/2) - 1)].animate.set_color(BLUE), data[(int(N/2)):(N-1)].animate.set_color(YELLOW),
                  data2[0:(int(N/2) - 1)].animate.set_color(BLUE), data2[(int(N/2)):(N-1)].animate.set_color(YELLOW))
        self.wait(2)
        ...
        reg_lin1 = Line(start=axes.c2p(-0.5, intercept - beta * 0.5 + delta),
                        end=axes.c2p(10, intercept + beta * 10 + delta),
                           color=YELLOW)

        reg_lin12 = Line(start=axes2.c2p(-0.5, intercept - beta * 0.5 + delta),
                        end=axes2.c2p(10, intercept + beta * 10 + delta),
                        color=YELLOW)
        ...
        beta_0 = MathTex("\\beta_0 = ", np.round(beta_0_hat, 2)).set_color(GOLD).move_to(axes.c2p(2, 9)).scale(0.75)
        beta_02 = MathTex("\\beta_0 = ", np.round(beta_0_hat, 2)).set_color(GOLD).move_to(axes2.c2p(2, 9)).scale(0.75)
        ...
        for i in range(0, I):
            ...
            beta_hat_i.append(MathTex("\\hat{\\beta}_0 = ", np.round(beta_i[0], 3)).set_color(RED).move_to(axes.c2p(9, 1.5)).scale(0.75))
            ...
            chosen2[:, i] = np.hstack((np.sort(np.array(rnd.sample(range(0, (int(N / 2) - 1)), k=(int(4 / 6 * N / 2))))),
        ...
        self.wait()
        self.play(FadeOut(gen_reg_lin, gen_reg_lin2, reg_lin_i, gen_reg_lin, beta_hat_i[I - 1], reg_lin_i2,
                          gen_reg_lin2, beta_hat_i2[I - 1], data, data2, beta_0, beta_02))
        self.wait()
        ...
        self.play(ReplacementTransform(text1, text4), ReplacementTransform(text2, text5), ReplacementTransform(text3, text6))
        self.wait()
